...
var Pergs = [],Perg
var EnterButton,RealButton,InteiroButton, ChooseButton, D=0
...
function preload(){ 
    ...
    //for(Aa;Aa!=0;Aa=Math.round(random(-9,9)));for(Bb;Bb!=0;Bb=Math.round(random(-9,9)))
...
function setup() {
    ...
    if(Button == 0){
    ChooseButton = createButton("Escolha")
    ...
    RespButton = createButton("Resposta")
    RespButton.position(windowWidth/2-windowWidth/2.5 + InteiroButton.width, windowHeight/2-windowHeight/2 )
...
break;
        case(6):Perg = new Form(Pergs[6]);Perg.input.remove();a+=1
